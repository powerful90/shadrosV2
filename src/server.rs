ClientMessage::GetAgents => {
    let agents_update = ServerMessage::AgentsUpdate { 
        agents: Vec::new() // Will be updated by listener
    };
...
ClientMessage::ExecuteCommand { agent_id, command } => {
    ...
    // In a full implementation, this would integrate with the listener
    let response = ServerMessage::Success { 
        message// src/server.rs
...
use crate::listener::{Listener, ListenerConfig, get_all_agents, add_task_for_agent};
